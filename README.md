...
`/deals` では、楽天から直近48時間以内に取得した公開商品を、期間限定セール、送料無料、
商品別ポイントアップに自動分類する。終了した販促情報や古い取得情報は表示しない。
...
セール・送料無料・商品別ポイントアップ一覧を有効にする場合は、019・020の後に
`supabase/migrations/021_product_promotions.sql` を実行します。次回の商品取り込みから
楽天の販促条件が保存され、`/deals`へ自動反映されます。既存商品を推測で販促対象にはしません。
...
3段階は詳細導線の初回計測以降の共通期間(最大28日)に揃え、開始7日未満は改善判定を出しません。
