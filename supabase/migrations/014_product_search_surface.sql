-- 商品検索結果の掲載面を匿名計測へ追加する。
-- 検索語は保存せず、商品ID・表示位置・時刻だけを既存と同じ方式で記録する。
...
alter table outbound_clicks add constraint outbound_clicks_surface_check
  check (surface is null or surface in ('home', 'category', 'feature', 'region', 'related', 'search', 'product'));
...
alter table product_impressions add constraint product_impressions_surface_check
  check (surface in ('home', 'category', 'feature', 'region', 'related', 'search'));
