...
create table outbound_clicks (
  ...
  surface text check (surface is null or surface in ('home', 'category', 'feature', 'region', 'related', 'search', 'product')),
...
create table product_impressions (
  id bigserial primary key,
  product_id uuid not null references products(id) on delete cascade,
  surface text not null check (surface in ('home', 'category', 'feature', 'region', 'related', 'search')),
  surface_key text check (surface_key is null or surface_key ~ '^[a-z0-9][a-z0-9-]{0,63}$'),
...
left join (
  ...
  from outbound_clicks
  where clicked_at >= now() - interval '28 days'
    and surface in ('home', 'category', 'feature', 'region', 'related', 'search')
  group by product_id
...
create view surface_position_performance_28d
with (security_invoker = true) as
with impression_totals as (
  select
    surface,
