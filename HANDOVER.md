...
- ヒノマルシェの楽天成果を正しく分離するには、新規メディア登録と専用リンクへの切替が必要。
  アカウント設定変更に当たるため、ユーザーの追加承認なしには実行しない。
...
1. ユーザー承認後、もしもへヒノマルシェを新規メディア登録し、専用楽天リンクへ切り替えて
   yamatoll実績と分離する（アカウント・本番設定変更のため未承認では実行しない）
2. 2026-07-23以降かつ個別商品100表示以上で匿名ファネルを再集計。未達なら変更しない
3. 承認済み断定語152件のうち58件は再判定済み。Anthropicクレジット追加後、残り94件
   （上限概算47円）を再開し、完了後に素材産地339件・入力外事実44件を続けるか再判断する
