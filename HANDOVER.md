...
- 有料再判定は58/152件成功した時点でAnthropic APIのクレジット残高不足となり停止。成功58件は
  ...
  shadowランキング最新日は2026-07-16のまま。クレジット追加は支払い変更のためユーザー操作待ち。
...
1. ユーザー承認後、`MOSHIMO_A_ID`のローカル・本番設定と楽天商品2,920件（公開937件）の
   既存リンクをヒノマルシェ専用IDへ切り替え、yamatoll実績と分離する
2. 2026-07-23以降かつ個別商品100表示以上で匿名ファネルを再集計。未達なら変更しない
3. 承認済み断定語152件のうち58件は再判定済み。Anthropicクレジット追加後、残り94件
   （上限概算47円）を再開する。ユーザーは2026-07-18にクレジット追加予定。完了後に
   素材産地339件・入力外事実44件を続けるか再判断する
4. Amazonは`hinomarche-22`の条件を満たす売上を待ち、過去30日10件到達後にCreators APIを再確認する
5. Search Consoleが28日100表示または個別ページ30表示へ達したら、クエリ・ページ別にSEO改善を判断する
6. 防災・菓子・ペット・波佐見焼は内部候補のまま保持し、検索表示またはサイト内反応が増えるまで
   新規ガイドを公開しない
7. 検討中の未決事項: スコア帯フィルタの「高(80%〜)」とバッジ色の「赤(90%〜)」の境界不一致を
   揃えるか(ユーザー未回答)
