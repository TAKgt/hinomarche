...
- Chromeに保存済みの3つのGoogleアカウントでは、`sc-domain:hinomarche.com` の
  Search Consoleプロパティへアクセスできなかった。
- 再確認条件: 所有者アカウントでのログイン、または現在使うアカウントへの閲覧権限付与後。
  権限が戻り次第、次回の高需要ページ選定前に検索クエリ・ページ別の表示回数とクリックを確認する。
- 権限待ちの間は公開在庫、販売先レビュー、需要スコアを代替根拠にする。2026-07-16時点では
  日本茶(24件・レビュー合計16,608件・需要スコア平均63)、鉄フライパン(誤分類除外後5件・
  7,160件・82)、お米(誤分類除外後12件・12,669件・62)を優先し、
  波佐見焼(24件・140件・47)は次点とした。

### 匿名計測の判断待ち(2026-07-16)

- 共通計測期間は2026-07-16 22:32 JST開始で、監査時点は開始0日。商品100表示以上は0件。
- 掲載面の商品表示96、一覧からの商品詳細閲覧2、販売サイト移動0。公開表示確認のアクセスも
  含み得るため、この初期値から改善判断を出さない。
...
- 改善案: 素材産地の明記がない場合は`material=unknown`、入力にない企業史・認定・性能を
  補わない、断定語を避ける、根拠文を確認できる事実2〜3点に絞る。既存商品の有料再判定と
  判定基準変更はユーザー確認後に行う。
...
- `contact_messages` は適用済みで保存可能。2026-07-13受信の未読1件があるため、
  管理者が内容を確認して対応する。
