...
- もしもアフィリエイトのトップで7月クイックレポート（42クリック/878表示、発生0円/承認0円）を
  読めたが、選択中メディア名が旧名称「通勤ラッシュが大嫌い」であり、ヒノマルシェとの対応を
  確定できない。売上レポートへ移動するとログイン画面へ戻ったため、楽天の30日/90日・
  プロモーション別実績は未取得。次回はChromeで再ログインし、このメディアがヒノマルシェ用かを
  確認してから読み取り専用で集計する。メディア名やレポート設定は変更していない。
- 2026-07-17の再確認でもChromeタブはログイン画面のまま。ID・パスワードは入力せず、
  ユーザーが同タブでログインした後に媒体対応と30日/90日・プロモーション別を続行する。
...
- 2026-07-17に本番DBを読み取り専用で再集計し、公開937商品・断定語候補152件・上限概算76円を
  再確認した。AI API呼び出しとDB書き込みは行っておらず、152件の実行はユーザー承認待ち。
...
1. もしもアフィリエイトへChromeで再ログインし、旧名称メディアとヒノマルシェの対応を確認した上で、
   楽天の30日/90日・プロモーション別実績を読み取る（設定変更なし）
2. 2026-07-23以降かつ個別商品100表示以上で匿名ファネルを再集計。未達なら変更しない
3. 有料再判定を行う場合は、まず断定語152件（概算76円）の実行承認を得る。完了後に差分を監査し、
