...
- 有料再判定は58/152件成功した時点でAnthropic APIのクレジット残高不足となり停止。成功58件は
  禁止断定語0件、保存スコア変更0件、featured score変更0件。残り94件（上限概算47円）は未処理。
  shadowランキング最新日は2026-07-17。クレジット追加は支払い変更のためユーザー操作待ち。
...
2. 承認済み断定語152件のうち58件は再判定済み。Anthropicクレジット追加後、残り94件
   （上限概算47円）を再開する。ユーザーは2026-07-18にクレジット追加予定。完了後に
   素材産地339件・入力外事実44件を続けるか再判断する
